...
rigid_::rigid_(math::vec2 center_vec, float val_mass, float val_friction, float val_restitution, float w, float h) {
...
void rigid_::on_update_gravity() {
    abstract_rigid::on_update_gravity();
}

void rigid_::on_update_position() {
...
uint8_t rigid_::get_vertex_count() {
    return this->vertex_count;
}

uint8_t rigid_::get_edge_count() {
    return this->edge_count;
}

void rigid_::project_to_axis(math::vec2 &axis, float &min, float &max) {
}

void rigid_::on_center_calc() {
    abstract_rigid::on_center_calc();
}

void rigid_::set_pos(math::vec2 center_pos) {
...
math::vec2 &rigid_::get_pos() {
    return this->center;
}

void rigid_::update_mass(float delta) {
    float val_mass = 0.0f;
...
void rigid_::move(math::vec2 vec) {
    this->minx = 10000.0f;
    this->miny = 10000.0f;
    this->maxx = -10000.0f;
    this->maxy = -10000.0f;
    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        this->minx = std::min(this->minx, this->vertex[i].x);
        this->miny = std::min(this->miny, this->vertex[i].y);
        this->maxx = std::max(this->maxx, this->vertex[i].x);
        this->maxy = std::max(this->maxy, this->vertex[i].y);
    }

    this->center += vec;
}

void rigid_::rotate(float val_angle) {
...
void rigid_::find_support_point(math::vec2 dir, math::vec2 edge) {
    math::vec2 to_edge;
    float projection;
...
bool rigid_::find_axis_least_penetration(rigid_ *&r, geometry::concurrent_collision_info &collision_info) {
